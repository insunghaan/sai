...
host=(root/'pair-host.js').read_text()
host_css=(root/'pair-host.css').read_text()
inner='<!doctype html><html lang="ko"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>사이 커플 앱</title><style>'+css+'</style></head><body><div id="root"></div><div id="toast" role="status"></div><dialog id="dialog"></dialog><script>'+model+'</script><script>'+app+'</script></body></html>'
...
scenarios=[('calendar','데이트 정하기','デートを決める'),('care','배려 주고받기','気づかい'),('pulse','심박 터치','心拍タッチ'),('breathe','함께 호흡','一緒に呼吸'),('walk','같이 걷기','一緒に歩く'),('date','데이트 기록','デート記録'),('garden','수면 정원','睡眠ガーデン'),('rhythm','편한 시간','都合のいい時間')]
outer='''<!doctype html><html lang="ko"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#ecefe5"><title>사이 SAI — 둘의 리듬, 두 개의 화면</title><style>'''+host_css+'''</style></head><body data-active="sumin"><header class="host-header"><a class="host-logo" href="index.html">sai<span>.</span></a><div class="host-brand-copy">TWO RINGS.<br>ONE LITTLE WORLD.</div><nav class="host-links">'''+tr('티저 보기 ↗','ティザーを見る ↗','a','href="teaser.html"')+tr('기존 모바일 ↗','以前のモバイル ↗','a','href="mobile-single.html"')+'''<button id="reset-top" aria-label="체험 초기화">↺</button><button id="host-language">KR / JP</button></nav></header><section class="host-intro"><small>OUR RHYTHMS, SIDE BY SIDE</small>'''+tr('나의 작은 선택이, 너의 하루에 닿도록.','わたしの小さな選択が、あなたの一日に届く。','h1')+tr('두 사람의 화면을 직접 눌러보세요. 한쪽에서 시작하고, 다른 쪽에서 함께 완성해요.','ふたりの画面を操作してみて。一方で始めて、もう一方で一緒に完成させます。','p')+'''</section><nav class="scenario-nav" aria-label="시연할 기능">'''+''.join(tr(ko,ja,'button',f'data-scenario="{k}"') for k,ko,ja in scenarios)+'''</nav><p class="scenario-guide" id="scenario-guide">아래 화면에서 자유롭게 체험하거나, 위에서 시연할 기능을 골라보세요.</p><nav class="mobile-switch" aria-label="체험할 사람"><button data-phone="sumin" class="selected" aria-pressed="true">🌱 '''+tr('수민의 화면','スミンの画面')+'''<i class="count" data-count="sumin" hidden></i></button><button data-phone="jiwoo" aria-pressed="false">🍑 '''+tr('지우의 화면','ジウの画面')+'''<i class="count" data-count="jiwoo" hidden></i></button></nav><main class="dual-stage">'''
...
for name in ['pair.html','mobile.html']:
