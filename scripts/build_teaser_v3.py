"""Build the independent, offline SAI teaser v3. Preserve teaser.html and app sources."""
...
ring_image=base64.b64encode((root/'assets/sai-rings-realistic-v1.png').read_bytes()).decode()
ring='<img class="ring-photo" src="data:image/png;base64,'+ring_image+'" width="1254" height="1254" alt="" decoding="async"><span class="art-spark" aria-hidden="true">✳</span><span class="art-spark tiny" aria-hidden="true">✧</span><span class="ring-tag" aria-hidden="true">you &amp; me.</span>'
...
page='''<!doctype html><html lang="ko"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#f9faf5"><meta name="description" content="매일의 잘 잤어가 조금 더 다정해지는 방법. 사이 SAI 커플 스마트링과 둘만의 작은 일상을 만나보세요."><title>사이 SAI — 둘만의 작은 세계 · Teaser v3</title><style>'''+css+'''</style></head><body data-entry="teaser"><div id="root"></div><div id="toast" role="status"></div><dialog id="modal"></dialog><script>'''+js+'''</script></body></html>'''
(root/'teaser-v3.html').write_text(page)
print(f'Built teaser-v3.html: {len(page.encode()):,} bytes (offline; existing teaser preserved)')
